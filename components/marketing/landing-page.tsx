...
export function LandingPage({ content }: { content?: PublicContent }) {
  ...
  const activeGalleryItem = liveGallery.find((item) => item.id === activeGallery);
      ...
      <main className="overflow-hidden bg-[#050505] text-white">
        ...
        <Section id="gallery" eyebrow="Gallery" title="Screenshots, mockups, previews, projects, and videos">
          <div className="columns-1 gap-4 sm:columns-2 lg:columns-3">
            {liveGallery.map((item, index) => (
              ...
              >
                {item.type === "video" ? (
                  <video src={item.src} muted playsInline className={cn("w-full object-cover transition duration-700 group-hover:scale-105", index % 3 === 0 ? "aspect-[4/5]" : "aspect-[4/3]")} />
                ) : (
                  <img src={item.src} alt={item.title} className={cn("w-full object-cover transition duration-700 group-hover:scale-105", index % 3 === 0 ? "aspect-[4/5]" : "aspect-[4/3]")} />
                )}
                <div className="p-4">
        ...
        <Section eyebrow="Wedding Reel Showcase" title="Short-form reels for cinematic event storytelling">
          <div className="flex gap-4 overflow-x-auto pb-4">
            {liveReels.map((item) => (
              <motion.div key={`reel-${item.id}`} {...reveal} className="group relative h-[520px] min-w-[280px] overflow-hidden rounded-lg border border-white/10 bg-[#111111]">
                {item.type === "video" ? (
                  <video src={item.src} autoPlay muted loop playsInline className="h-full w-full object-cover opacity-75 transition duration-700 group-hover:scale-105 group-hover:opacity-95" />
                ) : (
                  <img src={item.src} alt={item.title} className="h-full w-full object-cover opacity-75 transition duration-700 group-hover:scale-105 group-hover:opacity-95" />
                )}
      ...
      {activeGalleryItem && (
        <div className="fixed inset-0 z-[80] grid place-items-center bg-black/88 p-4 backdrop-blur-xl" role="dialog" aria-modal="true">
          ...
          {activeGalleryItem.type === "video" ? (
            <video src={activeGalleryItem.src} controls autoPlay className="max-h-[82vh] w-full max-w-5xl rounded-lg object-contain" />
          ) : (
